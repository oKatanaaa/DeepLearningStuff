...
class TheanoNN(object):
    ...
    def fit(self, Xtrain, Ytrain, Xtest, Ytest, epochs=1000, learning_rate=0.00001, batch_sz=100, reg=0.0, decay=0.999):
        # Create caches
        cache_W1 = theano.shared(np.zeros_like(self.W1.get_value()))
        cache_b1 = theano.shared(np.zeros_like(self.b1.get_value()))
        cache_W2 = theano.shared(np.zeros_like(self.W2.get_value()))
        cache_b2 = theano.shared(np.zeros_like(self.b2.get_value()))
        
        updated_cache_W1 = decay*cache_W1 + (1 - decay)*T.grad(self.cost, self.W1)**2
        updated_cache_b1 = decay*cache_b1 + (1 - decay)*T.grad(self.cost, self.b1)**2
        updated_cache_W2 = decay*cache_W2 + (1 - decay)*T.grad(self.cost, self.W2)**2
        updated_cache_b2 = decay*cache_b2 + (1 - decay)*T.grad(self.cost, self.b2)**2
        ...
        updated_W1 = self.W1 - learning_rate*T.grad(self.cost, self.W1) / T.sqrt(eps_W1 + updated_cache_W1)
        updated_b1 = self.b1 - learning_rate*T.grad(self.cost, self.b1) / T.sqrt(eps_b1 + updated_cache_b1)
        updated_W2 = self.W2 - learning_rate*T.grad(self.cost, self.W2) / T.sqrt(eps_W2 + updated_cache_W2)
        updated_b2 = self.b2 - learning_rate*T.grad(self.cost, self.b2) / T.sqrt(eps_b2 + updated_cache_b2)
        
        train = theano.function(
            inputs=[self.thX, self.thT],
            updates=[
                # Updated caches
                (cache_W1, updated_cache_W1), (cache_b1, updated_cache_b1), 
                (cache_W2, updated_cache_W2), (cache_b2, updated_cache_b2),
                # Update weights
