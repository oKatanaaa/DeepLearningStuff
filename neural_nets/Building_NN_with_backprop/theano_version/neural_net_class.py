...
class TheanoNN(object):
    ...
    def train(self, Xtrain, Ytrain, Xtest, Ytest, epochs=1000, learning_rate=0.00001, batch_sz=100, reg=0.0):
        updated_W1 = self.W1 - learning_rate*T.grad(self.cost, self.W1)
        updated_b1 = self.b1 - learning_rate*T.grad(self.cost, self.b1)
        updated_W2 = self.W2 - learning_rate*T.grad(self.cost, self.W2)
        updated_b2 = self.b2 - learning_rate*T.grad(self.cost, self.b2)
        
        train = theano.function(
            inputs=[self.thX, self.thT],
            updates=[(self.W1, updated_W1), (self.b1, updated_b1), (self.W2, updated_W2), (self.b2, updated_b2)]
        )
...
def main():
    ...
    costs, errors = model.train(Xtrain, Ytrain, Xtest, Ytest, epochs=1000)
